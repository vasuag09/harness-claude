...
1. **In-repo first** — does this codebase already solve a near-identical problem? Use
   the knowledge graph / `mgrep` to find existing patterns, utilities, and conventions.
2. **Library docs** — use **context7** to confirm current API/behavior of candidate
   libraries (don't answer from memory; versions drift).
...
4. **Broader web** — only if the above are insufficient (`mgrep --web "..."`).
...
## Exit criterion
A documented build-vs-reuse decision with evidence. Fold it into the spec. Then `/plan`.
