---
name: build-fix
description: Resolve build failures and type/compile errors fast, with minimal diffs. Use when a build or typecheck fails during implementation. Delegates to the build-error-resolver agent.
---
...
## Do this
1. **Delegate to `build-error-resolver`**, or inline:
...
## Escalate
If the only real fix is a design change, stop and return to `/architect` or `/plan`
rather than hacking around it.

## Exit criterion
Build, types, and lint are clean. Resume `/implement` or proceed to `/review`.
